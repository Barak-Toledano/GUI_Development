...
def show_about_info():
    messagebox.showinfo(
        title="About",
        message="The Barak Text Editor is a simple tabbed text editor designed to help you lean Tkinter."
    )

root = tk.Tk()
root.title("Barak Text Editor")
root.option_add("*tearOff", False)  # stop from turnoff from the interface to somewhere else in the screen

main = ttk.Frame(root)
main.pack(fill="both", expand=True, padx=1, pady=(4, 0))  # when no side, it is top by default
